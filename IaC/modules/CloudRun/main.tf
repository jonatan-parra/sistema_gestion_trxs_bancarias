...
resource "google_cloud_run_service" "usuarios-ms" {
  name     = "usuarios-ms"                         # Nombre del servicio de Cloud Run
  location = "us-east4"                              # Región donde se desplegará el servicio

  template {
    spec {
      containers {
        image = "us-east4-docker.pkg.dev/cbse-2024ii-438703/usuarios-ms/usuarios-ms:latest"  # URL de la imagen de contenedor en Artifact Registry
        ports {
        ...
        env {
          ...
          value = "34.21.55.243"                       # Reemplazar "IP_ADDRESS" por la IP pública de Cloud SQL
        }
        env {
...
resource "google_cloud_run_service_iam_member" "allow_unauthenticated" {
  location = google_cloud_run_service.usuarios-ms.location  # Ubicación del servicio Cloud Run
  service  = google_cloud_run_service.usuarios-ms.name      # Nombre del servicio Cloud Run al que aplica el IAM
